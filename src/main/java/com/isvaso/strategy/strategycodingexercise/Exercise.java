package com.isvaso.strategy.strategycodingexercise;
...
class QuadraticEquationSolver
{
    ...
    public Pair<Complex, Complex> solve(double a, double b, double c)
    {
        ...
        Complex c1 = new Complex(-b, 0);
        Complex c2 = new Complex(2.0 * a, 0);

        Complex plus = Complex.plus(c1, root).divides(c2);
        Complex minus = root.times(
                new Complex(-1.0, 0.0)).plus(c1).divides(c2);
...
class Main {
    public static void main(String[] args) {
        QuadraticEquationSolver q1
                = new QuadraticEquationSolver(
                        new OrdinaryDiscriminantStrategy());

        System.out.println(q1.solve(1, -1, -1).toString());
    }
}
